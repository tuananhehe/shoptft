...
export const TFTShop: React.FC<TFTShopProps> = ({ onSelectAccount }) => {
  ...
  return (
    <section id="shop" className="scroll-mt-14 sm:scroll-mt-20 pt-6 pb-8 sm:pt-16 sm:pb-16 lg:pt-20 lg:pb-20 bg-[#F8F9FA] border-t border-slate-200/90 border-b border-slate-200/90 text-slate-900 relative overflow-hidden">
      ...
      {/* 2. NÚT "XEM THÊM KHO ACC" TINH GỌN (Tối ưu chuẩn Mobile & Desktop) */}
        ...
        >
          ...
          <span>
            {showFullCatalog ? (
              ...
              <>
                <span className="sm:hidden">Xem toàn bộ kho ({vipAccounts.length}+ acc)</span>
                <span className="hidden sm:inline">Xem thêm toàn bộ kho acc ({vipAccounts.length}+ acc có sẵn)</span>
              </>
            )}
          </span>
          {showFullCatalog ? (
