...
export const TFTAccountModal: React.FC<TFTAccountModalProps> = ({ account, onClose }) => {
  const [selectedPackage, setSelectedPackage] = useState<PackageKey>("2h");
...
- Gói thuê: ${activePkg.name} (${activePkg.sub})
...
Báo mình khi acc này hết giờ thuê nhé!`;
    ...
    toast.success("Đã sao chép yêu cầu đặt trước! Hãy dán (Ctrl+V) vào Zalo để shop xếp lịch.");
    window.open(PROFILE_INFO.zaloUrl, "_blank");
  };
          ...
          {isRented ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-1.5 px-3 py-2 bg-blue-50/70 border border-blue-200/60 rounded-xl text-[11px] text-blue-900">
                ...
                <span>
                  <strong>Đặc quyền:</strong> Thuê gói 30 ngày chỉ cần bù thêm 70% để nâng cấp lên Thuê Lâu Dài bất cứ lúc nào.
                </span>
              </div>
            </div>
          )}
