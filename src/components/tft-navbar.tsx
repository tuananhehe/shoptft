...
export const TFTNavbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200/60 text-slate-800 shadow-sm transition-all">
      ...
      {/* TĂNG PADDING TỔNG THỂ CỦA HEADER (px-6 sm:px-8 lg:px-12) */}
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        {/* SINGLE ROW HEADER - GIÃN ĐỀU KHOẢNG CÁCH (gap-4 sm:gap-6 lg:gap-8) */}
        <div className="h-16 sm:h-18 flex items-center justify-between gap-4 sm:gap-6 lg:gap-8">
          {/* 1. LOGO & BRAND */}
          ...
          {/* 2. MENU LINKS NGẮN GỌN (Khoảng cách gap-6 lg:gap-8 thông thoáng) */}
          <nav className="hidden xl:flex items-center gap-6 lg:gap-7 text-xs font-semibold text-slate-600">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
          ...
          <div className="hidden md:flex flex-1 max-w-xs lg:max-w-sm relative">
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full h-9 pl-9 pr-3 bg-slate-100/90 focus:bg-white text-slate-900 placeholder-slate-400 text-xs rounded-full border border-slate-200/80 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/15 transition-all shadow-inner"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 flex-shrink-0">
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-sky-700 bg-sky-50 hover:bg-sky-100 border border-sky-200/80 rounded-xl transition-all shadow-sm"
            >
            ...
            <a
              href="#shop"
              className="inline-flex items-center gap-1.5 px-4 sm:px-4.5 py-2 bg-orange-600 hover:bg-orange-700 active:bg-orange-800 text-white font-bold text-xs sm:text-xs rounded-xl shadow-sm hover:shadow-md transition-all hover:scale-105"
            >
